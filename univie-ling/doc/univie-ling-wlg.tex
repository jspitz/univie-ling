...
By default, the class uses by default \emph{Crimson} as a serif font (via the \textsf{cochineal} package), \emph{Alegreya Sans} as sans serif,
and \emph{DejaVuSans Mono} as monospaced font.
Alternatively, the quite similar shaped \emph{MinionPro} (serif) and \emph{MyriadPro} (sans) can also be used.
They cover more glyphs and are probably a bit more polished than \emph{Cochineal} and \emph{Alegreya Sans}, but due to license reasons
...
\begin{itemize}
  \item Forthcoming (v.\,2.1)
	\begin{itemize}
		\item Add a sans (\emph{Alegreya} or \emph{MyriadPro}) and monospaced (\emph{DejaVuSans Mono}) font.
	\end{itemize}
