...
It is also possible, however, (though not recommended) to use the \texttt{bibliography} environment if,
...
\end{lstlisting}
%
(Only) if \textsf{natbib} is loaded, \uvlt\ will take care that the spacing with the \texttt{bibliography} environment
is correct.
...
\begin{itemize}
  \item Forthcoming (v.\,2.8)
	\begin{itemize}
		\item Add basic support for \texttt{bibliography} environment.
	\end{itemize}
