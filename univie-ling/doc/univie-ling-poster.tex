...
on whether the poster is filled evenly, or even if the text overruns the poster page. In the latter case, you could
watch our for \emph{Overfull vbox} warnings, but these are quite ambiguous.

To enhance accessibility in this regard, \uvlt\ provides the following helper function:
\begin{itemize}
	\item \verb|\restspace[<label>]|
\end{itemize}
%
If you insert this to your poster, it will tell you through a package warning in the log file how many lines there are still
available (in the current column, if you are using columns) as of the point where the command has been inserted.
With lines, the warning refers to lines in the normal font size of running text. 
In a second warning, the package tells you its own evaluation on how to deal with the result. Either of these three
warnings will be given:
\begin{enumerate}
	\item \enquote{Good, still space left}: poster/column is not full yet, you can safely add more text
	\item \enquote{It is still OK, but the poster is quite full}: The content gets to the boundary of the page.
	       If you don't have space available in other columns and need all text, you can keep this, but it will
	       probably look a bit crowded. So if you can, re-arrange.
...
So if you insert, for instance \verb|\restspace[1]|, the first warning will read 
\enquote{Remaining space (1): <X> normal lines} (with <X> being the actual number of lines).

Note that the \verb|\restspace| macro does not print anything on the poster itself, so you can safely keep
